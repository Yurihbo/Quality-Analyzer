...
function ScoreCard({ label, score, icon: Icon }: { label: string; score: number; icon: typeof Gauge }) {
  return <div className="rounded-xl border border-white/[0.08] bg-black/20 p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-medium text-zinc-400"><Icon className="h-4 w-4 text-zinc-600" />{label}</div><span className={`font-mono text-lg font-medium ${scoreTone(score)}`}>{score}</span></div><div className="mt-4 h-1 overflow-hidden rounded-full bg-white/[0.07]"><div className={`h-full rounded-full ${scoreBar(score)}`} style={{ width: `${score}%` }} /></div></div>;
}
...
function ReportView({ report }: { report: AnalysisReport }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 pb-24 pt-8 sm:px-8 lg:px-10">
      ...
      <section className="border-b border-white/[0.08] py-8" id="overview"><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-600">Overview</p><h2 className="mt-1 text-xl font-semibold text-zinc-100">Signal, not noise.</h2></div><span className="font-mono text-xs text-zinc-600">{report.summary.requests} requests observed</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><ScoreCard label="Performance" score={report.scores.performance} icon={Gauge} /><ScoreCard label="SEO" score={report.scores.seo} icon={Search} /><ScoreCard label="Security" score={report.scores.security} icon={ShieldCheck} /><ScoreCard label="Accessibility" score={report.scores.accessibility} icon={Sparkles} /><ScoreCard label="Best practices" score={report.scores.bestPractices} icon={Radar} /><ScoreCard label="Errors" score={report.scores.errors} icon={AlertCircle} /></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="stat-cell"><span>Issues</span><strong>{report.summary.errors}</strong></div><div className="stat-cell"><span>Warnings</span><strong>{report.summary.warnings}</strong></div><div className="stat-cell"><span>Technologies</span><strong>{report.summary.technologies}</strong></div><div className="stat-cell"><span>Elements</span><strong>{report.summary.elements}</strong></div></div></section>
...
function TopBar({ onReset }: { onReset: () => void }) {
  return <header className="topbar"><button onClick={onReset} className="brand-mark" aria-label="Quality Analyzer home"><span className="brand-square">Q</span><span>Quality <em>Analyzer</em></span></button><nav className="topnav"><button className="topnav-active" onClick={onReset}>Analyze</button><button onClick={() => undefined}>History</button><button onClick={() => undefined}>Compare</button><button onClick={() => undefined}>About</button></nav><div className="topbar-status"><span className="status-pulse" />Passive mode</div></header>;
}
